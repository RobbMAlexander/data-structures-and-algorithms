...
const toLastNames = people => {
  let fullNameArr = people.map(name => name = (name.firstName + ' ' + name.lastName));
  return fullNameArr;
};
...
const addPurchases = (arr) => {
  let totalCost = arr.reduce((total, product) => {
...
const countNumberOfElements = (arr) => {
  let scenicLength = arr.reduce((total) => {
    total++;
    return total;
  }, 0);
  return scenicLength;
...
const returnNames = (arr) => {
  let nameList = arr.reduce((namesArr, character) => {
...
const reversedString = (str) => {
  let strArr = [...str];
  let mirrorStr = strArr.reduce((reversed, strChar) => {
    return strChar + reversed;
  }, '');
  return mirrorStr;
};
