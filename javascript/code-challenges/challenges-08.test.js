...
const containsWorld = (input) => {
  let worldCheck = /world/;
  return worldCheck.test(input);
};
...
const isCapitalized = (str) => {
  let capitalCheck = /[A-Z][a-z]*/g;
  let capitalWasChecked = str.match(capitalCheck);
  if (capitalWasChecked) {
    return capitalWasChecked;
  } else {
    return [];
  }
};
...
const citiesAtoJ = (arr) => {
  let aThruJCities = [];
  let aThruJCheck = /(?<! )[A-J][a-z]*/g;
  arr.forEach(str => {
    if (aThruJCheck.test(str) === true) {aThruJCities.push(str)};
  });
  return aThruJCities;
};
