...
const maxInArray = (arr) => {
  let maxCheck = arr.reduce((prev, next) => {
    if (next > prev) {
      prev = next;
    }
    return prev;
...
const checkValues = (obj, value) => {
  ...
  return (objValues.some(element => element === value));
};
...
const hasChildrenValues = (arr, character) => {
  ...
  arr.forEach(char => {
    if (char.name === character) {
      Object.keys(char).forEach((property) => {
