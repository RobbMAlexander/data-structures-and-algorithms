...
const addTwo = (arr) => {
  let doubleIncrementArr = arr.map(num => num +2);
  return doubleIncrementArr;
};
...
const typeNum = (arr) => {
  let numsOnlyArr = arr.filter(element => typeof element === 'number');
  return numsOnlyArr;
};
...
const containsAnd = (arr) => {
  let stringsWithAndArr = arr.filter(string => string.includes('and') === true);
  return stringsWithAndArr;
};
...
const oddValues = (arr) => {
  let oddArr = arr.filter(value => (value % 2 === 0) === false);
  return oddArr;
};
...
const notInFirstArray = (forbiddenValues, arr) => {
  let redactedArr = arr.filter(value => forbiddenValues.includes(value) === false);
  return redactedArr;
};
