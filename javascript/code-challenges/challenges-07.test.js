...
const joinArray = (arr) => {
  let spaghettifiedArr = '';
  arr.forEach(element => {
    spaghettifiedArr = spaghettifiedArr + element.toString() + ' ';
  });
  spaghettifiedArr = spaghettifiedArr.slice(0, -1);
  return spaghettifiedArr;
};
...
const wordsToCharList = (arr) => {
  ...
  for (let i = 0; i < arr.length; i++) {
    ungluedStrArr.push(arr.slice(i, i + 1));
  }
  return ungluedStrArr;
};
...
const listFoods = (recipe) => {
  ...
  recipe.ingredients.forEach(ingredient => {
    slicedOnceArr.push(ingredient.slice(ingredient.indexOf(' ')+1, ingredient.length));
    return slicedOnceArr;
  });
  slicedOnceArr.forEach(ingredient => {
    result.push(ingredient.slice(ingredient.indexOf(' ')+1, ingredient.length));
  });
  return result;
};
