...
const addQuestion = (arr) => {
  let questionArr = arr.map(str => str + '?');
  return questionArr;
};
...
const forLoopTwoToThe = (arr) => {
  ...
  for (let i = 0; i < arr.length; i++) {
    poweredArr.push(Math.pow(2, arr[i]));
  }
  return poweredArr;
};
/* ------------------------------------------------------------------------------------------------
...
const forEachTwoToThe = (arr) => {
  let poweredArr = [];
  arr.forEach((num) => {
    poweredArr.push(Math.pow(2, num));
  });
...
const mapTwoToThe = (arr) => {
  let twoPoweredArr = arr.map(num => Math.pow(2, num));
  return twoPoweredArr;
};
