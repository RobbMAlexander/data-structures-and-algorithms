...
const replaceZeros = (string) => {
  const regexZeroIn = /0/g;
  return string = string.replace(regexZeroIn, 'zero');
};
...
const validatePin = (pin) => {
  let fourNumCheck = /^[0-9]{4}$/;
  return fourNumCheck.test(pin);
};
...
const validateWord = (word) => {
  let fiveToTenCharCheck = /^[a-zA-Z]{5,10}$/;
  return fiveToTenCharCheck.test(word);
};
...
const hasNumber = (string) => {
  let hasNumCheck = /[a-zA-Z]+[0-9]+/;
  return hasNumCheck.test(string);
};
...
const validatePhoneNumber = (phoneNumber) => {
  let validPhoneCheck = /^((\([0-9]{3}\))|([0-9]{3}))(-|\s*)[0-9]{3}(-|\s*)[0-9]{4}$/g;
  return validPhoneCheck.test(phoneNumber);
};
